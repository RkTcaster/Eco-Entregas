...
PARAM$experimento <- "RStestcv3f" #Random seeds
...
# Parámetro variable
semillas <- sample(seq(150000, 800000), 50, replace = FALSE)
...
for (semilla in semillas) {
  ...
  # Entrenar un nuevo modelo con el mejor número de iteraciones
  ...
  tb_importancia <- as.data.table(lgb.importance(model))
...
# Guardo el archivo
archivo_salida <- paste0(PARAM$experimento, "_predicciones_semillas_cv_3_final.csv")
